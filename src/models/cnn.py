...
class LazyConv(torch.nn.Sequential):
    ...
    def __init__(self, layers: th.List[th.Tuple[int, int]], conv_type: str, **kwargs):
        super().__init__()
        for idx, items in enumerate(layers or []):
            ...
            self.add_module(
                f"block_{idx}",
                LazyConvBlock(out_features=features, output_padding=output_padding, conv_type=conv_type, **args),
            )
